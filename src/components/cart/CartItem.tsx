...
import { IProductItem } from '../catalog/CatalogTypes';
...
export default function CartItem(props: IProductItem) {
  ...
  const quantityHandler = (type: 'dec' | 'inc') => {
    if (quantity === 1 && type === 'dec') {
      itemCartRef.current?.classList.add('animation-remove');
      setTimeout(() => {
        dispatch(quantityItemCart({ type, id }));
      }, 500);
    } else {
      dispatch(quantityItemCart({ type, id }));
    }
  ...
  return (
    <div ref={itemCartRef} className="cart__item">
